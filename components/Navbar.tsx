...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row items-center sm:items-center justify-center gap-0 sm:gap-4">
        ...
        <span className="font-serif text-sm sm:text-xl md:text-2xl font-medium text-white tracking-widest uppercase whitespace-nowrap mt-1 sm:mt-0">Creators Oak</span>
      </div>
            ...
            {isDesktopMenuOpen && (
              ...
              >
                <div className="flex flex-col py-3 font-sans text-sm sm:text-base font-medium">
                  ...
                  <button onClick={() => { setIsDesktopMenuOpen(false); setIsAboutModalOpen(true); }} className="group relative overflow-hidden text-left px-5 sm:px-6 py-2.5 text-white/90 transition-all duration-300 hover:text-[#dfb871] hover:bg-white/5">
                    <span className="absolute left-0 top-0 bottom-0 w-1 bg-[#dfb871] transform -translate-x-full group-hover:translate-x-0 transition-transform duration-300 ease-out" />
                    <span className="relative z-10 inline-block transform transition-transform duration-300 group-hover:translate-x-1">About Us</span>
                  </button>
